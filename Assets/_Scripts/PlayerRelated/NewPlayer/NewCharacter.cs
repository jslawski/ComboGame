...
public class NewCharacter : MonoBehaviour, DamageableObject {
	...
	float movespeed = 2;					//The player's max speed
	...
	float moveDampening = 0.50f;			//Dampen the player's movement when it reaches max speed
	...
	MeshRenderer thisMesh;                  //Reference to attached mesh renderer
	TextMesh thisText;						//Reference to the player's combo text
	...
	bool actionTaken = false;               //Whether or not the controller had any input on the current frame
	...
	void Start () {
		...
		abilities.Add(this.gameObject.AddComponent<NewDash>());
		maxHealth = 100;
		health = maxHealth;
	...
	void Update () {
		...
		if (curDevice.AnyButton.WasPressed || curDevice.LeftStick.Vector.magnitude != 0 || curDevice.RightStick.Vector.magnitude != 0) {
			actionTaken = true;
		}
		else {
			actionTaken = false;
